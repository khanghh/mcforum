...
func (s *articleService) Get(id int64) *model.Article {
	return repository.ArticleRepository.Get(sqls.DB(), id)
}

func (s *articleService) Take(where ...interface{}) *model.Article {
	return repository.ArticleRepository.Take(sqls.DB(), where...)
}

func (s *articleService) Find(cnd *sqls.Cnd) []model.Article {
	return repository.ArticleRepository.Find(sqls.DB(), cnd)
}

func (s *articleService) FindOne(cnd *sqls.Cnd) *model.Article {
	return repository.ArticleRepository.FindOne(sqls.DB(), cnd)
}

func (s *articleService) FindPageByParams(params *params.QueryParams) (list []model.Article, paging *sqls.Paging) {
	return repository.ArticleRepository.FindPageByParams(sqls.DB(), params)
}

func (s *articleService) FindPageByCnd(cnd *sqls.Cnd) (list []model.Article, paging *sqls.Paging) {
	return repository.ArticleRepository.FindPageByCnd(sqls.DB(), cnd)
}

func (s *articleService) Update(t *model.Article) error {
	err := repository.ArticleRepository.Update(sqls.DB(), t)
	return err
}

func (s *articleService) Updates(id int64, columns map[string]interface{}) error {
	err := repository.ArticleRepository.Updates(sqls.DB(), id, columns)
	return err
}

func (s *articleService) UpdateColumn(id int64, name string, value interface{}) error {
	err := repository.ArticleRepository.UpdateColumn(sqls.DB(), id, name, value)
	return err
}

func (s *articleService) Delete(id int64) error {
	err := repository.ArticleRepository.UpdateColumn(sqls.DB(), id, "status", constants.StatusDeleted)
	if err == nil {
...
func (s *articleService) GetArticleTags(articleId int64) []model.Tag {
	articleTags := repository.ArticleTagRepository.Find(sqls.DB(), sqls.NewCnd().Where("article_id = ?", articleId))
	var tagIds []int64
...
func (s *articleService) GetArticles(cursor int64) (articles []model.Article, nextCursor int64, hasMore bool) {
	...
	articles = repository.ArticleRepository.Find(sqls.DB(), cnd)
	if len(articles) > 0 {
...
func (s *articleService) GetTagArticles(tagId int64, cursor int64) (articles []model.Article, nextCursor int64, hasMore bool) {
	...
	nextCursor = cursor
	articleTags := repository.ArticleTagRepository.Find(sqls.DB(), cnd)
...
func (s *articleService) Publish(userId int64, form model.CreateArticleForm) (article *model.Article, err error) {
	...
	err = sqls.DB().Transaction(func(tx *gorm.DB) error {
		...
		if err = repository.ArticleRepository.Create(tx, article); err != nil {
			return err
		}
		repository.ArticleTagRepository.AddArticleTags(tx, article.Id, tagIds)
		return nil
	})
...
func (s *articleService) Edit(articleId int64, tags []string, title, content string, cover *model.ImageDTO) error {
	...
	err := sqls.DB().Transaction(func(tx *gorm.DB) error {
		...
		err := repository.ArticleRepository.Updates(sqls.DB(), articleId, updates)
		if err != nil {
			return err
		}
		tagIds, _ := repository.TagRepository.GetOrCreates(tx, tags)          // 创建文章对应标签
		repository.ArticleTagRepository.DeleteArticleTags(tx, articleId)      // 先删掉所有的标签
		repository.ArticleTagRepository.AddArticleTags(tx, articleId, tagIds) // 然后重新添加标签
		return nil
	})
...
func (s *articleService) PutTags(articleId int64, tags []string) {
	tagIds, _ := repository.TagRepository.GetOrCreates(sqls.DB(), tags)          // 创建文章对应标签
	repository.ArticleTagRepository.DeleteArticleTags(sqls.DB(), articleId)      // 先删掉所有的标签
	repository.ArticleTagRepository.AddArticleTags(sqls.DB(), articleId, tagIds) // 然后重新添加标签
	cache.ArticleTagCache.Invalidate(articleId)
}
...
func (s *articleService) ScanDesc(callback func(articles []model.Article)) {
	...
	for {
		...
		list := repository.ArticleRepository.Find(sqls.DB(), sqls.NewCnd().
