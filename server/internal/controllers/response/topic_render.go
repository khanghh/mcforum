...
func BuildTopic(topic *models.Topic, currentUser *models.User) *TopicResponse {
...
func BuildSimpleTopic(topic *models.Topic) *TopicResponse {
	buildContent := topic.Type == constants.TopicTypeTweet // 动态时渲染内容
	return _buildTopic(topic, buildContent)
}

func BuildSimpleTopics(topics []models.Topic, currentUser *models.User) []TopicResponse {
	if len(topics) == 0 {
		return nil
	}
...
func _buildTopic(topic *models.Topic, isBriefContent bool) *TopicResponse {
	if topic == nil {
		return nil
	}
	...
	if topic.NodeId > 0 {
		node := services.ForumService.Get(topic.NodeId)
