...
func BuildArticle(article *model.Article, currentUser *model.User) *ArticleResponse {
	...
	if currentUser != nil {
		rsp.Favorited = service.FavoriteService.IsFavorited(currentUser.Id, constants.EntityArticle, article.Id)
	}

	return rsp
}

func BuildSimpleArticle(article *model.Article) *ArticleSimpleResponse {
	if article == nil {
		return nil
	}

	rsp := &ArticleSimpleResponse{}
	rsp.Id = article.Id
	rsp.Title = article.Title
	rsp.Summary = article.Summary
	rsp.SourceUrl = article.SourceUrl
...
func BuildSimpleArticles(articles []model.Article) []ArticleSimpleResponse {
	if len(articles) == 0 {
		return nil
	}
	var responses []ArticleSimpleResponse
