...
// func (c *CommentController) Get() *web.JsonResult {
// 	var (
// 		err        error
// 		cursor     int64
// 		entityType string
...
func (c *CommentController) PostByReplies(commentId int64) *web.JsonResult {
	...
	comment, err := service.CommentService.CreateReply(user.Id, commentId, form)
	if err != nil {
		return web.JsonError(err)
	}
...
func (c *CommentController) GetByReplies(commentId int64) *web.JsonResult {
	cursor := params.FormValueInt64Default(c.Ctx, "cursor", 0)
	currentUser := service.UserTokenService.GetCurrent(c.Ctx)
...
func (c *CommentController) DeleteByReactionsBy(commentId int64, userId int64) (*web.JsonResult, error) {
	user := service.UserTokenService.GetCurrent(c.Ctx)
	if user == nil {
		return nil, service.ErrForbidden
	}
