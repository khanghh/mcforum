<template>
  <div class="gaming-card p-4 rounded-lg flex items-center gap-4">
    ...
    <div class="flex-1">
      ...
      <div class="text-xs text-gray-400 mt-1 line-clamp-1">{{ user.bio || '' }}</div>
    </div>
    <div>
      <button
        @click="toggleFollow"
        :class="isFollowing
          ...
          : 'px-2 py-0.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-md text-xs font-bold hover:scale-105 transition-transform'">
...
const api = (typeof useApi === 'function') ? useApi() : null
